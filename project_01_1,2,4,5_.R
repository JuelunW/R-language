...
# 4.2 
sig_m <- sd(R_OUT$SPY.Adjusted)

lot(density(s_2))

VaR <- mean(s_3) - quantile(s_3,0.05)
plot(density(s_3))
...
mu <- Mu[1:27]
Sigma <- var(R[, 1:27])*252
Sig_inv <- solve(Sigma)
vec_1 <- rep(1, 27)

w0 <- Sig_inv%*%vec_1/c(t(vec_1)%*%Sig_inv%*%vec_1)
mu_0 <- c(t(w0)%*%mu)
w1 <- Sig_inv%*%(mu - vec_1*mu_0)
sum(w1)
...
wa <- function(a){
  ...
  mu_p <- t(w)%*%mu
  sig_p <- t(w)%*%Sigma%*%w
  result <- c(mu_p = mu_p, sig_p = sig_p, w)
  return(result)
}

a_seq <- seq(0, 1, length = 100)
w_a_mat <- t(sapply(a_seq, wa))
ds <- data.frame(A = a_seq, w_a_mat)


png("5.1 summary plot.png")
plot(mu_p ~ sqrt(sig_p), type = "l", data = ds)
dev.off()

# 5.2
png("5.2 point.png")
plot(mu_p ~ sqrt(sig_p), type = "l", data = ds)
sr <- which.max(ds$mu_p/ds$sig_p)
points(mu_p[sr] ~ sqrt(sig_p)[sr], col = "blue", data = ds)
min <- which.min(ds$sig_p)
points(mu_p[min] ~ sqrt(sig_p)[min], col = "red", data = ds)
dev.off()
